...
function Carousel({photo, picLength}) {
    ...
    return (
      ...
      {photo?.length > 1 && (
        <img className="arrow arrow_left" src={Vector_left} alt="" onClick={handlePrevious}/>
      )}

      {photo?.map((photo, cle) => {
        return (
          <img key={cle} className={cle === index ? "carrousel_img actif" : "carrousel_img"} src={photo} alt=""/>
        );
      })}

      {photo?.length > 1 && (
        <img className="arrow arrow_right" src={Vector_right} alt="" onClick={handleNext}/>
      )}
